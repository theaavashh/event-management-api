...
const addEvent = wrapAsync(async (req: Request, res: Response) => {
...
const listEvent = wrapAsync(async (req: Request, res: Response) => {
...
cron.schedule(process.env.CRONJOB_STATUS as string, async () => {
	const dates = format(new Date(2024, 2, 18), "yyyy-MM-dd");
