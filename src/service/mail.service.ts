...
const sendingMail = async (paths: string, user: TUser) => {
	...
	status.accepted
		? logger.log("info", "Mail Send for verification")
		: logger.log("info", "Issue in send mail");
};
...
const updateEventMail = async (paths: string, arrData: IType[]) => {
	...
	const mailOption = {
		from: process.env.ACCESS_EMAIL as string,
		to: "aavash.bca_2018@padmashreecollege.edu.np",
	...
	const status = await transporter.sendMail(mailOption);
	status.accepted
		? logger.log("info", "Today Event Send To Mail")
		: logger.log("info", "Issue in Sending Mail");
};

const bookingEventMail = async (
	paths: string,
	fullname: string,
	email: string,
) => {
	...
	const status = await transporter.sendMail(mailOption);
	status.accepted
		? logger.log("info", "Sending Booking Confirmation Mail")
		: logger.log("info", "Issue in Sending Confirmation Mail");
};
